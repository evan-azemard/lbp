...
function envoiepanier($id_produit,$id_user,$nom_model,$resum,$prix,$nom_user,$nom_sellers,$id_sellers,$id_panier){
    $time = date('Y-m-d H:i:s');
    $bdd =  db_connect();
    $sql = $bdd->prepare("INSERT INTO commendes ( date_commende, id_user, id_produit, nom_article, resum_article, prix_article, nom_user, nom_seller, id_seller, id_panier) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
    $sql->execute(array( $time, $id_user,$id_produit,$nom_model,$resum,$prix,$nom_user,$nom_sellers,$id_sellers,$id_panier));
}
